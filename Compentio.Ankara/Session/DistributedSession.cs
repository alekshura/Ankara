namespace Compentio.Ankara.Session
{
    ...
    public class DistributedSession : ISession
    {
        ...
        {
            _cache.RemoveAsync(SessionKey).ConfigureAwait(false);
        }
        ...
        private void Load()
        {
            if (!_loaded)
            {
                try
                {
                    var data = _cache.Get(SessionKey);
                    if (data != null)
                    {
                    ...
                    {
                        _logger.AccessingExpiredSession(SessionKey);
                    }
                    _isAvailable = true;
                }
                catch (Exception exception)
                {
                    _logger.SessionCacheReadException(SessionKey, exception);
                    _isAvailable = false;
            ...
            if (!_loaded)
            {
                using (var timeout = new CancellationTokenSource(_ioTimeout))
                {
                    ...
                    try
                    {
                        ...
                        var data = await _cache.GetAsync(SessionKey, cts.Token);
                        if (data != null)
                        {
                            Deserialize(new MemoryStream(data));
                        }
        ...
        public async Task CommitAsync(CancellationToken cancellationToken = default)
        {
            using (var timeout = new CancellationTokenSource(_ioTimeout))
            {
                ...
                if (_isModified)
                {
                    if (_logger.IsEnabled(LogLevel.Information))
                    {
                        ...
                        try
                        {
                            ...
                            if (data == null)
                            {
                                _logger.SessionStarted(SessionKey, Id);
                            }
                        }
                    ...
                    try
                    {
                        ...
                        await _cache.SetAsync(
                            ...
                            cts.Token);
                        _isModified = false;
                        _logger.SessionStored(SessionKey, Id, _store.Count);
                    }
                    catch (OperationCanceledException oex)
                    {
                        if (timeout.Token.IsCancellationRequested)
                ...
                else
                {
                    try
                    {
                        await _cache.RefreshAsync(SessionKey, cts.Token);
                    }
                    catch (OperationCanceledException oex)
                    {
                        if (timeout.Token.IsCancellationRequested)
